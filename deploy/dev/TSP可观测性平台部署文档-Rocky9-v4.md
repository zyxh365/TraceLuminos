...
### 7.3 VictoriaMetrics 运维与使用

#### 7.3.1 Metrics 数据流

```
Java Agent（自动采集 JVM / HTTP / DB / Redis / Kafka 指标）
  │
  │ OTLP (/v1/metrics)
  ▼
Edge Collector ──Kafka (tsp-metrics)──▶ Central Collector
                                          │
...
> **数据自动采集**：Java Agent 已配置 `-Dotel.metrics.exporter=otlp`，会自动采集 JVM 指标（GC、内存、线程）、HTTP 指标（QPS、延迟）、数据库连接池、Kafka 消费 Lag 等，无需额外埋点。
...
# 关注字段：numSeries（序列数）、dataSize（磁盘占用）

# 查看 ingestion 统计
curl -s "http://localhost:8428/api/v1/status/ingestion" | python3 -m json.tool

# 查看存储路径和数据保留期
docker inspect tsp-victoriametrics | grep -A5 "Cmd"
# 期望看到 --storageDataPath=/victoria-metrics-data --retentionPeriod=90d
```

#### 7.3.3 VMui 可视化查询（浏览器）

浏览器访问 `http://<SERVER1_IP>:8428/vmui`

VMui 是 VictoriaMetrics 内置的 Metrics 可视化工具，功能类似 Grafana 的 Explore 面板：

- **查询面板**：输入 PromQL 表达式，实时查看图表
- **时间范围**：右上角选择时间范围（Last 5m / 1h / 6h / 1d / 自定义）
- **自动补全**：输入指标名时按 Ctrl+Space 自动提示
- **数据导出**：支持 CSV / JSON / 图表图片导出

#### 7.3.4 常用 PromQL 查询

以下查询可在 VMui 或 HTTP API 中直接使用。
...
```promql
# HTTP 请求 QPS（按服务 + 端点）
sum(rate(http_server_requests_seconds_count[5m])) by (service_name, http_route)

# HTTP 请求 P99 延迟（毫秒）
histogram_quantile(0.99,
  sum(rate(http_server_requests_seconds_bucket[5m])) by (le, service_name, http_route)
) * 1000
...
# HikariCP 连接等待时间
hikaricp_connections_pending_threads
```

**Kafka 消费 Lag：**

```promql
# Kafka 消费者 Lag（按 topic + 分区）
...
```promql
# 某个服务的全部指标概览（替换 service_name）
up{service_name="tsp-service-1"}

# 所有服务的请求数 Top 5
topk(5, sum(rate(http_server_requests_seconds_count[5m])) by (service_name))
```

#### 7.3.5 HTTP API 查询

在 tsp-monitor-gateway 或其他后端服务中，通过 HTTP API 查询 VictoriaMetrics：

```bash
# 即时查询（query）
curl -G "http://<SERVER1_IP>:8428/api/v1/query" \
  --data-urlencode 'query=up{job=~".*"}'

# 范围查询（query_range，返回时间序列，用于画图）
curl -G "http://<SERVER1_IP>:8428/api/v1/query_range" \
  --data-urlencode 'query=sum(rate(http_server_requests_seconds_count[5m])) by (service_name)' \
  --data-urlencode 'start=1744771200' \
  --data-urlencode 'end=1744774800' \
  --data-urlencode 'step=60'

# 查看所有指标名（发现可用指标）
curl -s "http://<SERVER1_IP>:8428/api/v1/label/__name__/values" | python3 -m json.tool

# 查看某个指标的所有标签值
curl -G "http://<SERVER1_IP>:8428/api/v1/label/service_name/values" \
  --data-urlencode 'match[]={__name__=~".*"}'
```

#### 7.3.6 tsp-monitor-gateway 集成

tsp-monitor-gateway（Server2 :8085）可通过 `<SERVER1_IP>:8428` 查询 VictoriaMetrics，用于在前端看板上展示 JVM 和应用指标。

Java 后端查询示例：

```java
// 查询 VictoriaMetrics 的 HTTP API
@Service
public class VictoriaMetricsService {

    @Value("${vm.endpoint:http://172.25.100.135:8428}")
     ...
     * 查询 JVM 堆内存使用
     */
    public Map<String, Object> getJvmHeapMemory(String serviceName) {
        String query = String.format(
            "jvm_memory_used_bytes{area=\"heap\",service_name=\"%s\"} / 1024 / 1024",
            serviceName);
        String url = String.format("%s/api/v1/query?query=%s", vmEndpoint,
            URLEncoder.encode(query, StandardCharsets.UTF_8));
        return restTemplate.getForObject(url, Map.class);
    }

    /**
     * 查询 HTTP QPS 时间序列（用于画图）
     */
    public Map<String, Object> getHttpQpsTimeSeries(String serviceName,
                                                      long start, long end, int step) {
        String query = String.format(
            "sum(rate(http_server_requests_seconds_count{service_name=\"%s\"}[5m]))",
            serviceName);
...
> **跨机访问**：确保 Server2 能访问 Server1 的 8428 端口。如果防火墙已按 2.7 节配置，则已开放。

#### 7.3.7 Metrics 数据保留策略

当前配置：

| 参数 | 值 | 说明 |
|------|-----|------|
| `retentionPeriod` | `90d` | 数据保留 90 天 |
| `storageDataPath` | `/victoria-metrics-data` | Docker volume `vm-data` |

如需调整保留期，修改 docker-compose.yml 中 VictoriaMetrics 的 `command` 参数：

```yaml
command:
  - "--storageDataPath=/victoria-metrics-data"
  - "--retentionPeriod=180d"   # 改为 180 天
```
...
#### 7.3.8 常见 Metrics 指标速查

OTel Java Agent 自动采集的指标（写入 VictoriaMetrics）：

| 指标名 | 说明 | 标签 |
|--------|------|------|
| `http_server_requests_seconds_*` | HTTP 请求延迟直方图 | service_name, http_route, http_method, status_code |
| `jvm_memory_used_bytes` | JVM 内存使用量 | service_name, area (heap/non-heap) |
| `jvm_memory_max_bytes` | JVM 内存最大值 | service_name, area |
| `jvm_gc_pause_seconds_*` | GC 暂停时间 | service_name, action, cause |
| `jvm_threads_current_threads` | 当前线程数 | service_name |
| `hikaricp_connections_*` | 数据库连接池 | service_name, pool |
| `process_cpu_usage` | CPU 使用率 | service_name |
| `process_runtime_jvm_uptime_seconds` | JVM 运行时间 | service_name |
| `kafka_consumer_consumer_lag` | Kafka 消费 Lag | service_name, topic, partition |

> 在 VMui 中输入 `http_server_requests_seconds` 后按 Ctrl+Space，可以看到该指标的所有可用标签和自动补全。
