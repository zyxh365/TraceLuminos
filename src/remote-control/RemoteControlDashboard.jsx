  ...
  return (
    <>
      {data.alerts && data.alerts.length > 0 && (
        <Section title="活跃告警"><AlertBanner alerts={data.alerts} /></Section>
      )}
      <Section title="端到端指令概览">
        ...
        <TimeSeriesChart height={180} series={[
    ...
    <>
      ...
      <Section title="响应延迟趋势 (ms)">
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
          <TimeSeriesChart height={240} yAxisLabel="ms" series={[
            { name: '平均', field: 'avg_duration' },
            { name: 'P99', field: 'p99_duration', color: COLORS.warning },
          ]} data={e2e.timeSeries} alertLine={{ value: 1000, label: '1s 告警线' }} />
  ...
  return (
    <>
      ...
      <Section title="QPS & 延迟趋势">
        <TimeSeriesChart height={240} yAxisLabel="QPS" series={[
    ...
    <>
      <Section title="核心指标">
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
          ...
          <GaugeChart title="处理延迟 P99" value={Number(svc.processingDelayP99Ms || 0).toFixed(0)} max={500} alertThreshold={ALERT_THRESHOLDS.tboxP99Delay} />
        </div>
      </Section>
      <Section title="延迟 & 失败趋势">
        ...
        ]} data={svc.timeSeries} alertLine={{ value: 200, label: '200ms 告警线' }} />
      </Section>
    </>
  );
}
    ...
    <>
      <Section title="核心指标">
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
          ...
          <GaugeChart title="短信唤醒成功率" value={svc.smsWakeupSuccessRate || 0} alertThreshold={ALERT_THRESHOLDS.smsWakeupSuccessRate} />
        </div>
      </Section>
      <Section title="短信趋势">
        <TimeSeriesChart height={240} series={[
      ...
      <Section title="MNO API 延迟趋势">
        <TimeSeriesChart height={180} yAxisLabel="ms" series={[
          { name: 'MNO延迟', field: 'mno_latency', color: '#00d4ff' },
        ]} data={svc.timeSeries} alertLine={{ value: 1000, label: '1s 告警线' }} />
      </Section>
    </>
  );
}
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 11, color: COLORS.muted, fontFamily: 'var(--mono)' }}>
          {data ? '数据已加载' : (loading ? '加载中...' : '')}{' · 自动刷新 30s'}
        </div>
      </div>
      ...
      {!error && (
        <>
          <div style={{ display: subTab === 'overview' ? 'block' : 'none' }}><OverviewPanel data={data} /></div>
          <div style={{ display: subTab === 'e2e' ? 'block' : 'none' }}><E2EPanel data={data} /></div>
          <div style={{ display: subTab === 'tsp' ? 'block' : 'none' }}><TspPanel data={data} /></div>
          <div style={{ display: subTab === 'tbox' ? 'block' : 'none' }}><TBoxPanel data={data} /></div>
          <div style={{ display: subTab === 'third-party' ? 'block' : 'none' }}><ThirdPartyPanel data={data} /></div>
          <div style={{ display: subTab === 'vehicle' ? 'block' : 'none' }}><VehiclePanel data={data} /></div>
        </>
      )}
    </div>
  );
