...
import { fetchTopology, searchTraceIds, fetchTraceDetail } from '../api/clickhouse.js';
...
function hashColor(name) {
  ...
  const sat = 65 + (Math.abs(h >> 8) % 20);
  const light = 55 + (Math.abs(h >> 16) % 15);
...
export default function ClickHouseTopology() {
  ...
  const buildTraceGraph = useCallback((spans) => {
    ...
    const edges = Object.entries(edgeMap).map(([key, items]) => {
      ...
      return {
        ...
        callCount: total, error_count: errors,
  ...
  useEffect(() => { const timer = setInterval(loadData, 60000); return () => clearInterval(timer); }, [loadData]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const now = Date.now();
      const results = await searchTraceIds({ keyword: id, searchType, startTime: now - lookback, endTime: now, limit: 50 });
      if (results.length === 0) {
        setSearchError(searchType === 'traceId' ? `未找到包含 "${id}" 的 TraceId` : `未找到包含 VIN "${id}" 的链路`);
        return;
      }
  ...
  const handleTraceClick = useCallback(async (traceId) => {
    if (selectedTrace === traceId) { setSelectedTrace(null); setTraceSpans(null); return; }
  ...
  const graph = isTraceMode ? buildTraceGraph(traceSpans) : (topologyData ? buildGraph(topologyData) : { nodes: [], edges: [] });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', background: 'rgba(13,21,32,0.9)', border: '1px solid #1e3a5f', borderRadius: 8, padding: '10px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <button onClick={() => { setSearchType('vin'); setSearchId(''); setSearchResults([]); setSearchError(null); }}
            style={{ padding: '4px 10px', borderRadius: 4, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: 11, background: searchType === 'vin' ? 'rgba(0,255,136,0.2)' : 'transparent', border: '1px solid '+(searchType==='vin'?'#00ff88':'#1e3a5f'), color: searchType==='vin'?'#00ff88':'#5a7090' }}>VIN码</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flex: 1, minWidth: 260 }}>
          <input value={searchId} onChange={e => setSearchId(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()}
            placeholder={searchType === 'traceId' ? '输入 TraceId 搜索...' : '输入 VIN 码搜索...'}
      ...
      {isTraceMode ? (
        <div style={{ background: 'rgba(176,106,255,0.08)', border: '1px solid #b06aff44', borderRadius: 6, padding: '8px 14px', fontFamily: 'var(--mono)', fontSize: 12, color: '#b06aff' }}>
          🔍 Trace 模式 — 展示该链路的服务调用链路（{nodes.length} 节点 · {edges.length} 边）
        </div>
      ) : (
      ...
      <div style={{ display: 'flex', gap: 14, height: '100%' }}>
        {/* 拓扑图 */}
        <div ref={svgContainerRef} style={{ flex: 1, minHeight: 560, height: '100%', background: 'rgba(8,13,20,0.95)', border: '1px solid #1e3a5f', borderRadius: 10, overflow: 'hidden', position: 'relative',
          ...(isFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, borderRadius: 0, border: 'none' } : {}) }}>
          <button onClick={toggleFullscreen} style={{ position: 'absolute', top: 12, right: 12, zIndex: 10, width: 28, height: 28, borderRadius: 6, cursor: 'pointer', background: 'rgba(30,58,95,0.6)', border: '1px solid #1e3a5f', color: '#5a7090', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, padding: 0 }} title={isFullscreen ? '退出全屏' : '全屏显示'}>⛶</button>

          {/* 图例 */}
          <div style={{ position: 'absolute', top: 12, left: 12, zIndex: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {(() => {
              const serviceNames = new Set();
              nodes.forEach(node => { const serviceName = node.id.includes(':') ? node.id.split(':')[0].trim() : node.id; serviceNames.add(serviceName); });
              return Array.from(serviceNames).map(serviceName => (
                <div key={serviceName} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: getColor(serviceName) }} />
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: '#5a7090' }}>{serviceName}</span>
                </div>
              ));
            })()}
          </div>
          <div style={{ position: 'absolute', top: 12, right: 48, zIndex: 10, fontFamily: 'var(--mono)', fontSize: 11, color: '#5a7090' }}>
          ...
          <svg ref={svgRef} width={svgSize.w} height={svgSize.h} style={{ display: 'block', userSelect: 'none', cursor: isDragging ? 'grabbing' : 'default' }}>
            ...
            {edges.map((e, i) => {
              ...
              const lx = (x1+x2)/2 - dy*0.06, ly = (y1+y2)/2 + dx*0.06;
              const isSelected = selectedNode && (e.source === selectedNode || e.target === selectedNode);
              const isTboxEdge = e.isTbox === true;
              const dashArray = isTboxEdge ? '6,4' : undefined;
              const tboxLabel = isTboxEdge ? '' : label;
              return (
                <g key={i}>
                  <path d={`M${x1},${y1} Q${mx},${my} ${x2},${y2}`} fill="none" stroke={color} strokeWidth={lineW+3} strokeOpacity={isSelected ? 0.2 : 0.08} strokeDasharray={dashArray} />
                  <path d={`M${x1},${y1} Q${mx},${my} ${x2},${y2}`} fill="none" stroke={color} strokeWidth={lineW} strokeOpacity={isSelected ? 0.9 : 0.55} markerEnd={`url(#arr-ch-${color.replace('#','')})`} strokeDasharray={dashArray} />
                  {!isTboxEdge && (
                    <><rect x={lx - label.length*3.2} y={ly-8} width={label.length*6.4} height={14} rx={3} fill="rgba(8,13,20,0.85)" stroke={color} strokeWidth={0.5} strokeOpacity={0.3} /><text x={lx} y={ly+1} textAnchor="middle" fill={color} fontSize="9" fontFamily="JetBrains Mono,monospace" opacity="0.9" fontWeight="500">{label}</text></>
                  )}
                </g>
              );
            })}
          ...
          <div style={{ flex: selectedTrace ? '0 0 auto' : 1, maxHeight: selectedTrace ? 220 : '100%', background: 'rgba(13,21,32,0.9)', border: '1px solid #1e3a5f', borderRadius: 10, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid #1e3a5f', fontFamily: 'var(--mono)', fontSize: 12, color: '#94a8c0', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {searchResults.length > 0
                ...
                : selectedNode
                  ? <><span style={{ color: getColor(selectedNode) }}>{selectedNode}</span><span style={{ color: '#5a7090' }}> 调用关系</span></>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              {searchResults.length === 0 && !selectedNode && (
                <div style={{ padding: 20, textAlign: 'center', color: '#5a7090', fontFamily: 'var(--mono)', fontSize: 11 }}>点击左侧节点查看调用关系</div>
              )}
              {searchResults.length === 0 && MIDDLEWARE.includes(selectedNode) && (
                <div style={{ padding: 14, color: '#5a7090', fontFamily: 'var(--mono)', fontSize: 11, lineHeight: 1.7 }}>中间件无独立 Trace。<br/>请点击调用它的服务节点。</div>
              )}
                ...
                return (
                  <div key={idx} style={{ padding: '10px 14px', borderBottom: '1px solid #0d1520' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: getColor(peer) }}>{isInbound ? '◀' : '▶'} {peer}</span>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: errorRate > 5 ? '#ef4444' : '#00ff88' }}>{errorRate > 0 ? `${errorRate.toFixed(1)}% 错误` : '✓ OK'}</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '1px 10px', fontFamily: 'var(--mono)', fontSize: 10 }}>
                      <span style={{ color: '#5a7090' }}>协议:</span><span style={{ color: '#c8d6e5' }}>{edge.protocol}</span>
                      <span style={{ color: '#5a7090' }}>调用次数:</span><span style={{ color: '#c8d6e5' }}>{edge.callCount.toLocaleString()}</span>
                      <span style={{ color: '#5a7090' }}>平均耗时:</span><span style={{ color: '#c8d6e5' }}>{fmtDuration(edge.avgDuration)}</span>
                      <span style={{ color: '#5a7090' }}>P99 耗时:</span><span style={{ color: '#c8d6e5' }}>{fmtDuration(edge.p99_duration_ms)}</span>
                      <span style={{ color: '#5a7090' }}>错误次数:</span><span style={{ color: errorRate > 5 ? '#ef4444' : '#c8d6e5' }}>{edge.error_count}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {selectedTrace && traceSpans !== null && (
            <div style={{ flex: 1, background: 'rgba(13,21,32,0.9)', border: '1px solid #1e3a5f', borderRadius: 10, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', borderBottom: '1px solid #1e3a5f', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={() => { setSelectedTrace(null); setTraceSpans(null); }} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#5a7090', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>×</button>
              </div>
          ...
          {topologyData && topologyData.length > 0 && (
            <div style={{ flex: '0 0 auto', background: 'rgba(13,21,32,0.9)', border: '1px solid #1e3a5f', borderRadius: 10, padding: '12px 14px', fontFamily: 'var(--mono)', fontSize: 11 }}>
              ...
              {(() => {
                ...
                const avgDur = edges.length > 0 ? edges.reduce((s, e) => s + (e.avgDuration || 0), 0) / edges.length : 0;
  ...
  return (
    <div onClick={() => onClick()} style={{ padding: '9px 14px', borderBottom: '1px solid #0d1520', cursor: 'pointer', background: selected ? 'rgba(0,255,136,0.08)' : 'transparent' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: hasError ? '#ff4d6a' : '#00ff88' }}>{hasError ? '✗ ERR' : '✓ OK'} · {fmtDuration(dur / 1000000)} · {trace.span_count} spans</span>
      </div>
      {bizVin && <div style={{ marginBottom: 3 }}><span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: '#b06aff', background: 'rgba(176,106,255,0.1)', borderRadius: 3, padding: '1px 4px' }}>VIN: {bizVin}</span></div>}
...
function ClickHouseSpanTree({ spans }) {
  ...
  if (!spans || spans.length === 0) return <div style={{ padding: 14, color: '#5a7090', fontFamily: 'var(--mono)', fontSize: 11 }}>无 Span 数据</div>;
  ...
  spans.forEach(s => {
    const pid = s.parent_span_id;
    if (!pid || pid === '' || !spanMap[pid]) roots.push(s.span_id);
    else (children[pid] = children[pid] || []).push(s.span_id);
  });
      ...
      <div key={sid}>
        <div style={{ paddingLeft: depth * 12, marginBottom: 5 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {hasKids ? <span onClick={() => toggle(sid)} style={{ color: '#00ff88', fontSize: 9, flexShrink: 0, cursor: 'pointer', userSelect: 'none', padding: '0 2px', lineHeight: '12px' }}>{isCollapsed ? '▶' : '▼'}</span>
              : depth > 0 ? <span style={{ color: '#1e3a5f', fontSize: 9, flexShrink: 0 }}>└</span> : null}
            {hasKids && !isCollapsed && <span style={{ fontSize: 8, color: '#3a5070', background: 'rgba(30,58,95,0.4)', borderRadius: 2, padding: '0 3px', flexShrink: 0 }}>{kids.length}</span>}
